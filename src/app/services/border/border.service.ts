...
export class BorderService {
  ...
  border(
    ...
    const right_char = comment_type.right
      ? comment_type.right
      : comment_type.char;
    ...
    const first_and_char_length_diff = comment_type.first
      ? comment_type.first.length - comment_type.char.length
      : 0;
    const border_width =
      (max_width + 1 + right_char.length - first_and_char_length_diff) /
      comment_type.char.length;
